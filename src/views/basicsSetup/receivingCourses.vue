        ...
        <div class="table-list">
            <el-table show-header border :data="tableList">
                ...
                <el-table-column label="操作" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-button type="text" size="small" @click="openCrriculum(scope.row)">设置领取课程</el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        ...
        >
            <el-table show-header border :data="crriculumList">
                <el-table-column
                    ...
                    align="center"
                ></el-table-column>
                <el-table-column label="是否可领取" align="center" fixed="right">
                    <template slot-scope="scope">
                        <el-checkbox
                            @change="e => updataCourses(e, scope.row)"
                            :value="scope.row.receive == 0"
                        />
                    </template>
                </el-table-column>
            </el-table>
...
export default {
    ...
    methods: {
            ...
            request
                ...
                .then(res => {
                    if (res.errno === 0) {
                        ...
                        this.getList();
                        this.getCurriculumList(this.openId);
                    } else {
                        this.$message({
                            type: "warning",
                            message: "设置失败!"
            ...
            request.post("/market/getList").then(res => {
                if (res.errno === 0) {
                    this.list = res.data.map(item => {
                        const list = item['curriculum_list'].map(val => val.receive);
                        return {...item, isReceive: list.includes('0')};
                    });
                }
            });
        },
        ...
        getCurriculumList(id) {
            request
                .post("/common/getCurriculumList", { id, type: 1 })
                .then(res => {
                    if (res.errno === 0) {
        ...
        delCurriculumList({ id }) {
            this.$confirm("您确定要删除？", "提示", {
                ...
                request
                    ...
                    .then(res => {
                        if (res.errno === 0) {
                            const data = this.crriculumList.filter(
                                item => id != item.id
                            );
